function [newTime, newSplineXYZ] = stretch3DCurve(refTime, factor, lenRef, refPointsWder, sampleFreq, order, zeroJerk)
%stretch3DCurve       Stretch 3D curve in time.
%     newTime, newSplineXY = stretch3DCurve(t,f,l,dp,s,o,z) returns the new time vector
...
%     indicated by repeated entries in dp. z indicates if it is desired zero jerk
%     as start and end conditions.
...
%     See also stretch1DCurve, stretch2DCurve 
    ...
    if ~zeroJerk
        ...
        dSplTimeRef = addValueAt(newTimeRef, [1, 1, 1], 0);
        lenTime = length(dSplTimeRef);
